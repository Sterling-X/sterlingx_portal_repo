...
// Backs both the dashboard's "Check Now" button (browser fetch, carries a
// NextAuth session cookie) and the daily 9AM ET Cloud Scheduler job (see
// docs/cron-setup.md), which has no session cookie -- it authenticates two
// ways stacked: an OIDC identity token verified by Cloud Run's own IAM
// layer before the request ever reaches this app (--no-allow-unauthenticated,
// see docs/deploy-commands.md), plus this route's own CHECKUP_CRON_SECRET
// check as a second, app-level factor -- matching the AI-Projects
// client-data-validator health-check route's convention. This uses a
// dedicated `X-Checkup-Cron-Secret` header, not `Authorization` -- Cloud
// Run's IAM layer already owns the `Authorization` header for the
// caller's OIDC identity token, so the app-level secret needs its own
// header or the two would collide. This route is excluded from
// src/middleware.ts's page-redirect gate precisely because it needs this
